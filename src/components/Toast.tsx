...
import { ToastContext } from "../contexts/ToastCotext";
...
const Toast = () => {
  ...
  const { state, dispatch } = toastContext;
  const show = state.toastMessage.length > 0;
  ...
  return show ? (
    <div
      ...
      role="alert"
    >
      <div className="text-sm font-normal">{state.toastMessage}</div>
